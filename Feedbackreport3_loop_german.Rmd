...
b <- ggplot() + theme_gray() +
  ...
  scale_x_continuous(labels=percent_format(accuracy = 1), limits = c(min(RR_figures[,2])-0.02, max(RR_figures[,2])+0.02)) +
  scale_y_continuous(labels=percent_format(accuracy = 1), limits = c(min(RR_figures[,3])-0.02, max(RR_figures[,3])+0.02)) +
  theme(legend.position = "right", legend.title = element_text(size = 8),
