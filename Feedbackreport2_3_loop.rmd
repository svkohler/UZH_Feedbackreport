...
b <- ggplot() + theme_gray() +
  ...
  scale_color_identity(name = NULL,
                          ...
                          guide = "legend") +
  labs(subtitle="Risk Vs Return", caption = "Annualized Volatility vs. Return during the investment period \nBlaBlaBla")
...
c <- ggplot(data=inv.style, aes(coeffs,factor))+geom_point(aes(col=y)) +
  ...
  labs(subtitle="Investment Style", caption = "Correlation of your investment style with different portfolios. \nThe black line references a correaltion of 1.")
...
print(knitr::kable(RRS_clean, format = 'latex', booktabs=TRUE)%>%
  kableExtra::pack_rows("Raw Returns", 1, 2) %>%
  kableExtra::pack_rows("Riskadjusted Performance Measures", 3, 5, latex_gap_space = "0.3cm")%>%
...
if(tail(WikiDaily_merge$EuroStoxx.normed, 1)<tail(WikiDaily_merge$close.normed, 1)){
  ...
  cat(paste0("You \\textbf{underperformed} compared to the benchmark. You came up short by ", round(tail(WikiDaily_merge$close.normed, 1)-tail(WikiDaily_merge$EuroStoxx.normed, 1), 2),"%. "))
}
